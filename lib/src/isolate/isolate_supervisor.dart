...
class IsolateSupervisor {
  ...
  final StreamController<IsolateServerInfo> _serverInfoController =
      StreamController.broadcast();
  ...
  Stream<IsolateServerInfo> get serverInfo => _serverInfoController.stream;
  ...
  void _handleMessage(dynamic message) {
    ...
    } else if (message is IsolateServerInfo) {
      _serverInfoController.sink.add(message);
    } else {
  ...
  Future<void> stop() async {
    if (_isStarted) {
      ...
      await _eventController.stream
          .firstWhere((element) => element is IsolateStopped);
